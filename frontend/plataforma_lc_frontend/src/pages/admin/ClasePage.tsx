...
import { getClase, createClase, updateClase, deleteClase } from '../../api/asistenciaApi'
...
function ClasesPage() {
  ...
  const handleSubmit = async (clase: Clase) => {
    try {
      ...
      const mensaje = err.response?.data || 'Error al registrar clase.'
      alert(mensaje)
    }
  }
            ...
            <TableBody>
              {clases.map(clase => (
                <TableRow key={clase.id}>
                  ...
                  <TableCell>{clase.id_curso || '—'}</TableCell>
                  <TableCell>
                    {clase.fecha
                      ? String(clase.fecha).split('T')[0]
                      : '—'}
                  </TableCell>
